...
def make_zst(records: list[dict]) -> bytes:
    """Serialise a list of dicts to a compressed NDJSON .zst blob."""
    ndjson = "\n".join(json.dumps(r) for r in records) + "\n"
    return zstd.ZstdCompressor().compress(ndjson.encode())


def write_zst(path, records: list[dict]) -> None:
...
@pytest.mark.asyncio
async def test_process_file_returns_dicts(tmp_path):
    ...
    fs = FileService()
    out = []
    async for obj in fs.process_file(str(p)):
        out.append(obj)
...
@pytest.mark.asyncio
async def test_process_file_single_record(tmp_path):
...
@pytest.mark.asyncio
async def test_process_file_many_records(tmp_path):
...
async def test_iter_file_bytes_chunk_boundary(tmp_path):
    ...
    out = []
    async for chunk in fs._iter_file_bytes(str(p)):
        out.append(chunk)
    assert out == [b"ab", b"cd", b"ef"]


@pytest.mark.asyncio
async def test_iter_file_bytes_empty_file(tmp_path):
...
@pytest.mark.asyncio
async def test_iter_file_bytes_chunk_size_larger_than_file(tmp_path):
